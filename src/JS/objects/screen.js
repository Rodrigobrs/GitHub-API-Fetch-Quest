const screen = {
    ...
    renderUser(user){
        ...
        user.repositories.forEach(repo => repositoriesItens +=`<li><a href="${repo.html_url}" target="_blank" >${repo.name}<div class="repo-data"><span>🍴${repo.forks_count}</span><span>🌟${repo.stargazers_count}</span><span>👁️‍🗨️${repo.watchers_count}</span><span>💻${repo.language ?? '?'}</span></div></a></li>` )
